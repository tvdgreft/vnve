...
class Forms
{
	...
	*/
	public $formdefaults = array(
		...
		'description' => 'check the checkbox'	#checkboxtext
	);
	#
	# button variables
...
return($html);
	...
	*/
	 ...
	 * class => class for other option (see forms.js-> CheckInput))
	 * error => Error when class detects an error
	 * required => (Boolean) Input is required (default) or not
	 * readonly => (Boolean) field is readonly
	 * collabel => bootstrap position of label (in case of inline=TRUE)
	 * col' => bootstrap position (inline = FALSE)
	...
	public function Check($args)
	{
		...
		$html .= '<input type="checkbox" id="' . $args["id"] . '" name="' . $args["name"] . '"';
		if($args['value']== 'on') { $html .= ' checked'; }
		$html .= '>';
		$html .= '<span class="error_hide">'.$args['error'].'</span>'; #span for error field see: forms.js
		$html .= '</div>';
		$html .= '</div>';
	...
	{	
		$args = $this->parse_args( $args, $this->formdefaults );
		$html='';
		$args['name'] = isset($args['name']) && $args['name'] ? $args['name'] : $args['id']; # name is id if not defined
		$html .= '<div class="form-group row">';
		$html .= 	'<div class="' . $args["collabel"] .'">';
		$html .= 		'<label for="' . $args["id"] . '"';
		$html .= '>' .  $args["label"] . '</label>';
		$html .= '</div>';
		$html .= 	'<div class="' . $args["colinput"] .'">';
		$html .= '<input type="checkbox" id="' . $args["id"] . '" name="' . $args["name"] . '" required value="checked">';
	...
	public function Time($args)
	{
		...
		$html .=	'</div>';
		$html .= 	'<div class="' . $args['colinput'] . '">';
		#$html .= sprintf('<input type="time" id=%s name=%s step="3600" min="00:00" max="23:59" pattern="[0-2][0-9]:[0-5][0-9]" style="width:%s" value="%s"',$args["id"],$args["id"],$args["width"],$args["value"]);
		$html .= '<input class="form-control ' . $args["checkclass"] . ' timepicker24" type="text" id="' . $args["id"] . '" name="' . $args["id"]. '" style="width:' . $args["width"] . '" value="' . $args["value"] .'"';
		#
		# set event on classes checkclass and datepicker in jquery !!
		#
		if($args['required']) { $html .= ' required="required"'; }
	 ...
	 * @param array $args[
	 * 'group' => dropdown will be followed by other form items (so div frorm-group is defined outside) = inline only
	 * 'label' => (string) Label of inputfield
	 * 'id' => (string) element id
	 * 'name' => (string) element name
	 * 'value' => (string) beginwaarde van het invoerveld
	...
	public function Dropdown($args)
	{
		...
		if($args['inline']== TRUE) 
		{
			if($args["group"] == FALSE) { $html .= '<div class="form-group row">'; }	#open group
			$html .= 	'<div class="' . $args["collabel"] .'">';	#div collabel
		}
		else 
		{ 
			$html .= '<div class="' . $args["col"] . '">';	#div col
			$html .= '<div class="control-label">';			#div control-label
		}
		$html .= 		'<label for="checkbox">' . $args["label"] . '</label>';
		$html .= 	'</div>';	#close control-label / colllable
		if($args['inline'] == TRUE) { $html .= '<div class="' . $args['colinput'] . '">'; }	#div controls
		else { $html .= '<div class="controls">';}
				#
		# pairs or options??
		#
		...
		if($args["group"] == FALSE) { $html .= '</div>'; } # close group
		return($html);
	}
	...
	public function WordpressMedia($args)
	{
		$html='';
		$args = $this->parse_args( $args, $this->formdefaults );
		if(!$args["name"]) { $args["name"] = $args["id"]; }		# als name niet gedefinieerd is name = id
		...
		#
		#$uploads = $args['uploads'];
		$photo_url = $args["value"];
		$html .= '<img id="image-view" src="' . $photo_url . '" width=150px height=100px alt="foto">';
		$html .= '<input id="image-url" type="hidden" name="' . $args['name']. '" value="' . $args["value"] .  '"/>';
		$html .= '<input id="upload-button" type="button" class="button image-button" value="Upload Image" />';
		$html .= '</div>';
		if($args['group'] == FALSE) { $html .= '</div>'; }
		return($html);
	}
	...
	*/
	...
	public function MakeForm($vargs)
	{
		$html = '';
		$options = new options();
        $params = $options->Parameters();						#read all parameters																   
        ...
        foreach($vargs['form'] as $fieldset)
        {
            ...
            {
				$a = $field->attributes();
				if($a->type == 'wordpresspages')
				{
					if(PRANA_CMS != "wordpress") { $html .= 'CMS is not wordpress!!' .'<br>'; continue;}
					#Make dropdown of wordpress pages
					$pages = get_pages();
					$options = '';
					foreach ($pages as $page) 
					{
						...
						{
     						$options .= $options != '' ? sprintf(",%s",$page->post_name) : sprintf("%s",$page->post_name);
						}
					}
					$a->options=$options;
					$a->type="dropdown";
				}
				if($a->type == 'wordpressmedia')
				{
					if(PRANA_CMS != "wordpress") { $html .= 'CMS is not wordpress!!' .'<br>'; continue;}
				}
				...
				if(isset($a->description)) { $args += ["description" => $a->description]; }		#used for checkboxes
				...
				if(isset($a->paramoptions)) { $args += ["options" => explode(',',$params[(string)$a->paramoptions])]; }	#defined in backend options
				if(isset($a->paramdirectory)) { $args += ["uploads" => $params[(string)$a->paramdirectory]]; }
				...
				switch ($a->type)
				{
					...
					case "dropdown":
					$html .= $this->DropDown($args);
					break;
					
					case "checkbox":
					$html .= $this->Check($args);
					break;
					...
					case "confirm":
					$html .= $this->ConfirmCheck($args);
					break;

					case "wordpressmedia":						#Get url of image in wordpressmebia library
					$html .= $this->WordPressMedia($args);
					break;
				}
            }
	...
	public function PrintForm($args)
	{
		...
		switch ($args["output"])
		{
			...
			case "pdf":
			require_once(PRANA_PLUGINPATH . '/vendor/'  . 'fpdf186/fpdf.php' );
			#
			# define constants
			#
			$topmargin=10;
