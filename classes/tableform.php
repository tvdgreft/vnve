...
class tableform
{
	...
	public $access;		#manager or public
	public $params;		#parameters given in options backend													  
	public $table;		#tablename
	public $logtable;	#table for logging
	public $primarykey;		#field with primary key of table
	public $template = "table";	#how to display the records as a table or list
	...
	public $searchtype = "string";	#search on string = complete string, "andwords" = andsearch on words, "orwords" orsearch on all words
	public $searchfilter;	#filter used withi searchtable (public search)
	public $searchonly;		#search only on records with this content
	public $searchexcept; 	#search onlu on records except this content
	...
	public $rows_per_page = 1000;	#maximum number of rows per page
	...
	public $permissions;	#permissions for maintaining table cr,md,dl,vw,cp(kopie maken),xp(export table)up(upload records)
	...
	public $carouseltype = "auto";
	public $norecordsfound = "Geen resultaten gevonden";
    ...
    {   
        ...
        {
			if(array_is_list($this->filtercolumns) == FALSE)
			{
			...
			{
				foreach ($this->filtercolumns as $cols)
				{
					$c = $cols[0];
				 	if(isset($_POST[$c])) { $filters[$c] = $_POST[$c]; }
				 	$this->filters = (object)$filters;
	 ...
	 * display columns, searchfield and searchcolumns
	 */
	public function DisplayTable()
	{
		...
		$html .= '<div class="row">';
		/**
		 * display search field if searchcolumns are  defined
		 */
		if($this->searchcolumns)
		{
			 ...
			 * search button
		 	*/
			$html .= sprintf('<button class="prana-btnsmall col" id="newsearch" name="newsearch">zoeken</button>');
		}
		/**
		...
		# combination of fields will be treated as AND search
		#
		if(isset($this->filtercolumns) && count($this->filtercolumns))
		{
			...
			$html .= '<p onclick="ToggleFilters(\'filterbox\')"><a class="prana-button">Uitgebreid zoeken</a></p>';
			#$html .= '</div>'; #end left part
			...
			#
			# old version search on text in all fields
			#
			$flag = TRUE;
			if(array_is_list($this->filtercolumns) == FALSE)
			{
			...
			#
			# new version
			# Columns to be filtered with dropdown possibility e.g.  $this->filtercolumns = [["Number","nummer","text"],["Coltype","collectie","dropdown"]];
			#
			else
			{
				foreach ($this->filtercolumns as $cols)
				{
					...
					if($type == "dropdown") 
					{ 
						$uvs=$dbio->UniqueValues(array("table"=>$this->table,"column"=>$c));
						$html .= $form->DropDown(array("label"=>$label, "id"=>$c, "value"=>$value, "options"=>$uvs));
					}
					else
					{
						$html .= $form->Text(array("label"=>$label, "id"=>$c, "value"=>$value, "popover"=>"info zoeken"));
		...
		$html .= '<br><br>';
		#
		# The header of the display . create button, exportbutton, initialload button
		#
		$html .= '<div style="float:left" class="row">';
		/**
		 * Upload records
		 */
		if(in_array("up", $this->permissions)) #initial load button if there are no records
		{
	...
	* yourColumnName regexp '(^|[[:space:]])yourWord([[:space:]]|$)';
	...
	public function DisplayRecords() : string
	{
		...
		# set pagenumber on 1
		#
		if(isset($_POST['lastsearch']) && $_POST['search'] && $_POST['search'] != $_POST['lastsearch'])
		{
			#echo '<br>reset page on search';
			$this->onpage=1;
		}
		if(isset($_POST['newsearch'])) #deze doet het niet als zoekveld is gewijigd
		{
			#echo '<br>reset page on newsearch';
			$this->onpage=1;
		}
		#
		# sort button clicked.
		...
		if($this->previouspage) { $this->onpage -= 1; } # previous page given so go the previous page
		if(isset($_POST['firstpage'])) { $this->onpage = 1; } # go back to first page
		#
		...
		$pb = $dbio->ReadRecords(array("table"=>$this->table,"columns"=>$this->columns,"filters"=>$this->filters,
										"search"=>array($this->searchcolumns,$this->onsearch,$this->searchtype),
										"searchfilter"=>$this->searchfilter,"searchonly"=>$this->searchonly,"searchexcept"=>$this->searchexcept));
		$this->rows=count($pb);
		/**
		...
		if(isset($_POST['lastpage'])) { $this->onpage = $this->pages; }
		...
		$pb = $dbio->ReadRecords(array("table"=>$this->table,"columns"=>$this->columns,"page"=>$this->onpage,"maxlines"=>$this->rows_per_page,
									...
									"searchfilter"=>$this->searchfilter,"searchonly"=>$this->searchonly,"searchexcept"=>$this->searchexcept));
		#
		# no records found skip table
		#
		if($this->rows == 0)
		{
			$html .= sprintf('<h2>%s</h2>',$this->norecordsfound);
			return($html);
		}
		if($this->template == 'table') { $html .= $this->TemplateTable($pb); }
		if($this->template == 'list') { $html .= $this->TemplateList($pb); }
		if($this->template == 'slides') { $html .= $this->TemplateSlides($pb); }
		if($this->template == 'carousel') { $html .= $this->TemplateCarousel($pb); }
		return($html);
	}
	/**
	 * The records will be displayed as a slideshow.
	 * all slides on one page
	 */
	public function TemplateSlides($pb)
	{
		$dbio = new dbio();		#class for database I/O
		$html = '';
		...
		foreach ( $pb as $p )
		{
			foreach($this->columns as $col)
			{
				...
				$name = $col[0];
				$type = $col[2] ? $col[2] : "string";	// default type is string
				if($type == 'viewer')		#the column viewer is the column where the document is defined to be shown
				{
					if($url = $this->DocumentExist($p->$primarykey,$p->$name))
					{
						$slide = '<figure>';
						if($p->doctype == "pdf")
						{
							$slide .= sprintf('<iframe class="prana-pdf" src="%s#page=1" type="application/pdf"',$url);
							$slide .= '></iframe>';
							$download = sprintf('<a href="%s">%s</a>',$url,$p->title);
							$slide .= sprintf('<figcaption class="prana-slidecaption">%s</figcaption>',$download);
						}
						#
						# The document is an image.
						# This image will be shown with a caption
						#
						if($p->doctype == "image")
						{
							$slide .= sprintf('<img class="prana-img" src="%s" type="application/pdf"',$url);
							$slide .= '></img>';
							#
							# make a caption 
							# with a link to a page of the current website
							#
							if($p->internallink)
							{
								if(strchr($p->internallink,"http") == FALSE) { $link = sprintf('<a href="%s%s">%s</a>',PRANA_HOMEURL . '/',$p->internallink,$p->title); }
								else { $link = sprintf('<a href="%s">%s</a>',$p->internallink,$p->title); }
							}
							#
							# or an external webpage
							#
							elseif($p->externallink)
							{
								$link = sprintf('<a href="%s">%s</a>',$p->externallink,$p->title);
							}
							#
							# or with no link
							#
							...
							$slide .= sprintf('<figcaption class="prana-slidecaption">%s</figcaption>',$link);	#onderschrift
						}
						$slide .= '</figure>';
						$html .= $slide;
					}
	...
	/**
	 * The records will be displayed in a carousel
	 * $this->carouseltype = "manual" or ""auto"
	 */
	public function TemplateCarousel($pb)
	{
		$dbio = new dbio();		#class for database I/O
		$html = '';
		$primarykey = $this->primarykey;

		if($this->carouseltype == "auto") { $html .= '<div class="owl-carousel owl-theme">'; }
		if($this->carouseltype == "manual") { $html .= '<div class="owl-manual owl-carousel owl-theme">'; }
		foreach ( $pb as $p )
		{
			foreach($this->columns as $col)
			{
				...
				if($type == 'viewer')		#the column viewer is the column where the document is defined to be shown
				{
					if($url = $this->DocumentExist($p->$primarykey,$p->$name))
					{
						...
						if($p->doctype == "pdf")
						{
							...
							$slide .= sprintf('<figcaption class="prana-slidecaption">%s</figcaption>',$download);
							$slide .= '</figure>';
							$html .= '<div class="itempdf">' . $slide . '</div>';
						}
						if($p->doctype == "image")
						{
							$slide .= sprintf('<img class="prana-img" src="%s" type="application/pdf"',$url);
							...
							$slide .= sprintf('<figcaption class="prana-slidecaption">%s</figcaption>',$link);	#onderschrift
							$slide .= '</figure>';
							$html .= '<div class="itemimg">' . $slide . '</div>';
						}
					}
				}
			}
		...
		$html .= '</div>';
		if($this->carouseltype == "manual")
		{
			$html .= '<div class="btns">';
			$html .= '<div class="customNextBtn"><i class="fa fa-arrow-right" aria-hidden="true"></i></div>';
			$html .= '<div class="customPreviousBtn"><i class="fa fa-arrow-left"></i></div>';
			$html .= '</div>';
		}
		return($html);
	}
	...
	/**
	 * The records will be displayed as a list. In fact in a 2-column table.
	 * The first column is the first column
	 * the first row in the second column is the second column
	 * The second 
	 */
	
	public function TemplateList($pb)
	{
		$dbio = new dbio();		#class for database I/O
		$html = '';
		/**
		 ...
		 */
		$html .= '<br>';
		$html .= '<table class="pranalisttable">';
		#
		# print rows
		#
		$primarykey = $this->primarykey;
		foreach ( $pb as $p )
		{
			...
			foreach($this->columns as $col)
			{
				$backslash=FALSE;				#scheidingsteken tussen velden plaatsen?
				$name = $col[0];
				$type = $col[2] ? $col[2] : "string";	// default type is string
				if($type == 'viewer')		#viewer make a new columns
				{
					if($opencell == TRUE) { $html .= '</td>';}		#should be on seperate column
					$toggleid="toggle".$p->id;
					$html .= '<td class="prana-link" onclick="ToggleRow(\''.$toggleid.'\')">'. $p->$name . '</td>';
					$opencell == FALSE;
				}
				elseif($type == 'download')
				{
					if($opencell == TRUE) { $html .= '</td>';}
					if($url = $this->DocumentExist($p->$primarykey,$p->$name))
					{
						$btn = '<i class="fa fa-download" style="font-size:24px;color:blue;"></i>';
						$html .= '<td><a href="'.$url.'" download>' . $btn . '</a></td>'; #download direct
					}
					else
					{
						$btn = '<i class="fa fa-download" style="font-size:24px;"></i>-';
						$html .= '<td>' . $btn . '</td>';
						#$html .= '<td></td>';
				...
				{
					if($opencell == FALSE )		#is de kolom met overige velden al geopend?
					{
						$html .= '<td>';
						$backslash=FALSE;
						$html .= $p->$name;
			...
			$type = $col[2] ? $col[2] : "string";	// default type is string
			$toggleid="toggle".$p->id;
			#$html .= '<td onclick="ToggleRow(\''.$toggleid.'\')"><a href="#">'. $p->$name . '</a></td>';
			$html .= '<td class="prana-link" onclick="ToggleRow(\''.$toggleid.'\')">'. $p->$name . '</td>';
			$html .= '<td>';	#second field on seperate line
			$col = next($columns);
			$name = $col[0];
			$type = $col[2] ? $col[2] : "string";	// default type is string
			$html .= $p->$name;
	...
	public function TemplateTable($pb)
	{
		$dbio = new dbio();		#class for database I/O
		$html = '';
		/**
		...
		foreach ( $pb as $p )
		{
			...
			foreach($this->columns as $c)
			{
				...
				if(($type == "pdf" || $type == "txt") && $p->$name)		#download a pdf file
				{
					...
					$download = $this->DocumentExist($p->$primarykey,$file);
					if($download == 0)	#document does not exists
					{
						$html .= '<td></td>';
					}
					elseif($download == -1)
				...
				elseif($type == "viewer")
				{
					$ext = prana_fileExtension($p->$name);
					if($this->DocumentExist($p->$primarykey,$p->$name))
					{
				...
				elseif($type == "download")
				{
					 ...
					 */
					$ext = prana_fileExtension($p->$name);
					$url = $this->DocumentExist($p->$primarykey,$p->$name);
					if($url == 0)
					{
					...
					elseif($url == -1)
					{
						$btn = '<i class="fa fa-download" style="font-size:24px;color:grey;"></i>-'.$ext;
						$html .= '<td class="compacttd">' . $btn . '</td>';
					}
					else
					{
				...
				elseif($type == "showbraille")    #show braille on screen
				{
					$path_parts = pathinfo($p->$name);
					$file = $path_parts['filename'] . '.' . "txt";
					if($this->DocumentExist($p->$primarykey,$file))
					{
				...
				elseif($type == "printbraille")    #print braille at CBB
				{
					$ext = prana_fileExtension($p->$name);
				...
				elseif($type == "downloadbraille")    #download the unicode braille
				{
					$ext = prana_fileExtension($p->$name);
					$path_parts = pathinfo($p->$name);
					$file = $path_parts['filename'] . '.' . "txt";
	...
	/**
	 * buttons for first, next,  previous and last page
	 */
	public function PageButtons()
	{
		...
		{ 
			...
			if($this->onpage > 1) 
			{ 
				$html .= '<button type="submit" class="btn btn-link btn-sx" name="firstpage" value="' . $this->onpage . '"><i class="fa fa-fast-backward" style="font-size:18px"></i></button>';
				$html .= '<button type="submit" class="btn btn-link btn-sx" name="previouspage" value="' . $this->onpage . '"><i class="fa fa-backward" style="font-size:18px"></i></button>';
			}
			if($this->onpage < $this->pages) 
			{ 
				
				$html .= '<button type="submit" class="btn btn-link btn-sx" name="nextpage" value="' . $this->onpage . '"><i class="fa fa-forward" style="font-size:18px"></i></button>';
				$html .= '<button type="submit" class="btn btn-link btn-sx" name="lastpage" value="' . $this->onpage . '"><i class="fa fa-fast-forward" style="font-size:18px"></i></button>';
			}
		}
		return($html);
	}
	...
	*/
	...
	public function SetPosts()
	{
		...
		#
		$html = '';
		if(isset($_POST['search']))	#moet weten of zoekveld is gewijzigd om onpage op 1 te zetten
		{
			$html .='<input id="lastsearch" name="lastsearch" type="hidden" value="' . $_POST['search'] .  '" />';	#current page
		}
